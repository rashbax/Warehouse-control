...
export async function createPrihod(formData: FormData): Promise<{ error?: string }> {
  const session = await auth();
  if (!session?.user?.id) redirect("/login");

  const skuId = formData.get("skuId") as string;
  if (!skuId) return { error: "Выберите товар." };
  ...
  if (!sku) return { error: "Товар не найден." };
  ...
  try {
    ...
    return { error: "Ошибка при записи прихода." };
  }
